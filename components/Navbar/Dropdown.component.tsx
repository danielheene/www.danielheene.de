...
import { WithChildren, WithClassName } from '@typings/common';
...
const StyledMenuItem = styled.a<Pick<MenuLinkProps, '$active'>>`
  ...
  ${({ $active }) =>
    $active
      ? tw`bg-gray-700 bg-opacity-50 text-white`
...
function MenuButtonIcon({
  ...
  if (type === 'right')
    return <RightIcon className={className} icon={icon} aria-hidden='true' />;
