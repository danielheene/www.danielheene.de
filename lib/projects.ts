 ...
 * under my `danielheene` username & then filter them down to only
...
async function fetchProjects(): Promise<Projects | null> {
  const response = await fetch(
    'https://api.github.com/users/danielheene/repos',
    {
      headers: {
  ...
  const projects: Projects = json
    .map((repo) => {
      ...
      const repoPost =
        ...
        projectPosts.find(
          (post) =>
            post.repository.toLowerCase() === repo.full_name.toLowerCase()
        );
      ...
      return {
        ...
        language: repo.language
          ? GithubColors.get(repo.language).color
          : undefined,
        name: repo.name,
